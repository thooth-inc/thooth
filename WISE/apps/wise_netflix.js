function netflixFetch(){
	
	var callUrl = WISE.server + "/index.php?action=ajax&rs=wfGetNetflixData&rsargs[]=" + encodeURIComponent(WISE.trim(thisApp.params[0])) + "&rsargs[]=netflixRender"
	WISE.call(callUrl, thisApp.id);

}

function netflixRender(j) {
	...
	var title = "Results from Netflix - " + theItem.title["@attributes"].regular;
	var url = "http://www.netflix.com";
	var rating = false;
	var genres = "";
...
function netflixRender(j) {
	...
	var title = "Results from Netflix - " + j.catalog_titles.catalog_title.title["@attributes"].regular;
	var url = "http://www.netflix.com";
	var rating = false;
	var genres = "";
	for (var i=0; i<j.catalog_titles.catalog_title.category.length; i++) {
		if (j.catalog_titles.catalog_title.category[i]["@attributes"] && j.catalog_titles.catalog_title.category[i]["@attributes"].scheme) {
			if(j.catalog_titles.catalog_title.category[i]["@attributes"].scheme.indexOf("mpaa_rating") > 0)	rating = j.catalog_titles.catalog_title.category[i]["@attributes"].term;
			else if (j.catalog_titles.catalog_title.category[i]["@attributes"].scheme.indexOf("genre") > 0) genres += j.catalog_titles.catalog_title.category[i]["@attributes"].term + "; ";
		}
	}
	var output = "<table><tr><td valign='top'><img src='" + j.catalog_titles.catalog_title.box_art["@attributes"].medium + "' /></td><td valign='top'>";
		output += "<span class='wise_netflix_headline'>" + j.catalog_titles.catalog_title.title["@attributes"].regular + " (" + j.catalog_titles.catalog_title.release_year + ") " + (rating ? "[" + rating + "]": "") + "</span>"
		output += (j.catalog_titles.catalog_title.average_rating ? "<span class='wise_netflix_rating'>Avg. User Rating: " + j.catalog_titles.catalog_title.average_rating + "</span>" : "");
		output += "<div>" + j.catalog_titles.catalog_title.link[0].synopsis + "</div>";
		output += (rating ? "<div><b>Genres:</b> " + genres.substring(0,65) + (genres.length > 65 ? "..." : "") + "</div>": "");
		output += "<a href='http://www.netflix.com'>Delivered by Netflix</a>";
	output +="</td></tr></table>"
	for(var i=0; i< j.catalog_titles.catalog_title.link.length; i++) {
		if(j.catalog_titles.catalog_title.link[i]["@attributes"].title == "web page") url = j.catalog_titles.catalog_title.link[i]["@attributes"].href;
	}
