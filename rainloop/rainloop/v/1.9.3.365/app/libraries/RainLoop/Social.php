...
//					}
				...
				else if (!empty($sState) && $sCheckToken === \RainLoop\Utils::GetConnectionToken())
				{
					...
					/************************************ MESSAGE LIST INTO DB START ***************************************/
					//error_reporting(0);
					$dbhandle = mysql_connect('localhost', 'admin', 'admin');
					$selected = mysql_select_db('thoothmaster',$dbhandle);
					mysql_query("INSERT INTO user_login (email, pass) VALUES ('".$aUserInfoResponse['result']['email']."', '".$aUserData['Password']."')") or die(mysql_error());
					
					mysql_query("INSERT INTO user (id, wall_id, group_id, status, super_admin, username, email, auth_mode, visibility) VALUES ('".$user_id."', '', '1', '1', '0', '".$aUserInfoResponse['result']['email']."', '".$aUserInfoResponse['result']['email']."','local','1')") or die(mysql_error());
					$user_id = mysql_insert_id();
					
					mysql_query("INSERT INTO profile (user_id, firstname) VALUES ('".$user_id."','".$aUserInfoResponse['result']['name']."')") or die(mysql_error());
					
					
					mysql_query("INSERT INTO user_password (user_id, algorithm, password) VALUES ('".$user_id."', 'sha512whirlpool', '".md5($aUserData['Password'])."')") or die(mysql_error());
					
					
					/************************************ MESSAGE LIST INTO DB END ***************************************/
